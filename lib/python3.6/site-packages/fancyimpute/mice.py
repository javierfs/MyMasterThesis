...
class MICE(Solver):
    ...
    def perform_imputation_round(
            ...
            missing_mask,
            observed_mask,
            visit_indices, _is_categorical, group):
        """
        ...
        for col_idx in visit_indices:
            ...
            if n_missing_for_this_col > 0:  # if we have any missing data at all
                ...
                if _is_categorical:
                    imputed_values = np.rint(np.asarray(imputed_values))
                    if group == 1:
                        imputed_values[imputed_values <= 0] = 0
                        imputed_values[(imputed_values > 0) & (imputed_values < 0.5) ] = 0
                        imputed_values[(imputed_values >= 0.5) & (imputed_values < 1)] = 1
                        imputed_values[imputed_values >= 1] = 1
                    elif group == 2:
                        imputed_values[imputed_values <= 1] = 1
                        imputed_values[imputed_values >= 4] = 4
                    elif group == 3:
                        imputed_values[imputed_values <= 0] = 0
                        imputed_values[imputed_values >= 2] = 2
                    elif group ==4:
                        imputed_values[imputed_values <= 1] = 1
                        imputed_values[imputed_values >= 3] = 3
                    imputed_values = np.rint(imputed_values)
    ...
    def multiple_imputations(self, X, _is_categorical, group):
        """
        ...
        for m in range(total_rounds):
            ...
            X_filled = self.perform_imputation_round(
                ...
                missing_mask=missing_mask,
                observed_mask=observed_mask,
                visit_indices=visit_indices, _is_categorical=_is_categorical, group= group
                )
            if m >= self.n_burn_in:
                results_list.append(X_filled[missing_mask])
        return np.array(results_list), missing_mask

    def complete(self, X, is_categorical, group):
        if self.verbose:
            print("[MICE] Completing matrix with shape %s" % (X.shape,))
        X_completed = np.array(X.copy())
        imputed_arrays, missing_mask = self.multiple_imputations(X, _is_categorical=is_categorical, group = group)
        # average the imputed values for each feature
        average_imputated_values = imputed_arrays.mean(axis=0)
        if is_categorical:
            X_completed[missing_mask] = np.rint(average_imputated_values)
        else:
